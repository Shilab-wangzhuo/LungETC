...
def make_parser():
    parser = argparse.ArgumentParser("ResNeXt Classifier")
    parser.add_argument("--test_dir", type=str, default=None)
    parser.add_argument("--save_dir", type=str, default=None)
    parser.add_argument("--ori_img_dir", type=str, default=None)
    parser.add_argument("--threshold", type=float, default=0.5)
    parser.add_argument("--weight", type=str, default="tools/Step6_malignant_benign_classification/weights/resnext.pth", help="ResNeXt model path")
...
class Test_model():
    def __init__(self, opt):
        ...
        self.model.eval()
        self.class_name = ['benign', 'malignant']  # 0=benign, 1=malignant
    
    def load_model(self, weight_path):
        # 创建模型实例
        model = CustomResNeXt(num_classes=2)
    ...
    def __call__(self):
        ...
        # 创建malignant子目录
        malignant_dir = os.path.join(self.save_dir, "malignant")
        os.makedirs(malignant_dir, exist_ok=True)
        
        # 创建malignant_ori子目录
        malignant_ori_dir = os.path.join(self.save_dir, "malignant_ori")
        os.makedirs(malignant_ori_dir, exist_ok=True)
            ...
            for imgpath in tqdm(img_list, desc="Processing Images"):
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        # 现在索引1对应malignant
                        malignant_prob = probs[0][1].item()  # 修改这里，使用索引1获取malignant概率
                    
                    # 根据阈值确定类别
                    if malignant_prob > self.threshold:
                        output_class = self.class_name[1]  # malignant，索引1
                        malignant_count += 1
                        
                        # 保存被分类为malignant的图像
                        img_save_name = f"{malignant_prob:.4f}_" + basename(imgpath)
                        img_save_path = os.path.join(malignant_dir, img_save_name)
                        img_save_path_ori = os.path.join(malignant_ori_dir, img_save_name)
                        ...
                        output_class = self.class_name[0]  # benign，索引0
                        benign_count += 1
                    
                    # 写入结果文件
                    file.write(f"{basename(imgpath)}\t{output_class}\t{malignant_prob}\n")
                
                except Exception as e:
                    print(f"Error processing {imgpath}: {e}")
            
            # 写入统计信息
            total = malignant_count + benign_count
            file.write(f"\nSummary:\n")
            file.write(f"Malignant: {malignant_count}\n")
            file.write(f"Benign: {benign_count}\n")
